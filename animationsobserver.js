...
let imgArr = Array.from(document.querySelectorAll('.img_obs'));
let paras = Array.from(document.querySelectorAll('.para'));
...
const imgObserver = new IntersectionObserver((e, imgObserver) => {
  e.forEach((e, i) => {
    if (e.isIntersecting) {
      e.target.classList.add('show_img');
      e.target.style = `animation-delay: ${i * 200}ms`;
      /* imgObserver.unobserve(e.target); */
    } else {
      e.target.classList.remove('show_img');
    }
  }, options);
});
...
const paraObserver = new IntersectionObserver((e, paraObserver) => {
  e.forEach((e) => {
    if (e.isIntersecting) {
      e.target.classList.add('show_p');
    } else {
      e.target.classList.remove('show_p');
    }
  }, options);
});
